...
def get_video_snippets(video_ids):
    ...
    df["is_short"] = df["duration_sec"] <= 180
    return df
...
def fetch_intro_text(video_id, chars=300):
    try:
        ...
        txt = " ".join(seg["text"] for seg in t)
        return txt[:chars]
    except:
        return ""
...
def find_top_matches(query, exclude_channel, duration_filter, top_n):
    params = {
        "part": "snippet",
        "q": query,
        "type": "video",
    ...
    return stats.sort_values("views", ascending=False).head(top_n).to_dict("records")

# — Sidebar —

channel_id = st.sidebar.text_input("Your Channel ID")
num_matches = st.sidebar.number_input(
    "Number of matches", min_value=1, max_value=10, value=5, step=1
)
title_thresh = st.sidebar.slider(
    "Title similarity threshold (%)", 0, 100, 50
)
intro_thresh = st.sidebar.slider(
    "Intro similarity threshold (%)", 0, 100, 50
)
thumb_thresh = st.sidebar.slider(
    "Thumbnail hash distance threshold", 0, 64, 10
)
...
if channel_id:
    ...
    chosen = st.selectbox(
        f"Select a {video_type} video",
        subset["title"].tolist()
    )

    if st.button("Run Similarity Search"):
        row = subset[subset["title"] == chosen].iloc[0]
        our_intro = fetch_intro_text(row["videoId"])
        our_hash = get_image_hash(row["thumbnail"])
